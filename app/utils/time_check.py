...
from app.repository.agents.agent_metrics_repository import save_execution_time
...
def time_check(func):
    """함수의 실행시간을 측정하는 데코레이터 함수 (자동으로 DB에 저장)"""
    ...
    if asyncio.iscoroutinefunction(func):
        async def wrapper(*args, **kwargs):
            ...
            async with async_session_maker() as session:
                await save_execution_time(func.__name__, execution_time, session)
