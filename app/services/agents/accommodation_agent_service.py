...
class AccommodationAgentService:
    ...
    def prepare_crew_inputs(self, user_input: dict):
        ...
        keywords = concepts + ([user_prompt] if user_prompt else [])
    ...
    async def create_accommodation_recemmendation(self, user_input: dict):
        """
        ...
        try:
            ...
            # 3. Crew 실행
            crew = Crew(tasks=tasks, agents=list(self.agents.values()), verbose=True, memory=True, process=Process.sequential)
